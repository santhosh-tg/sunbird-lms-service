...
class QuestionPublisherSpec extends FlatSpec with BeforeAndAfterAll with Matchers with MockitoSugar {
  ...
  val jobConfig: QuestionSetRePublishConfig = new QuestionSetRePublishConfig(config)
  ...
  "getExtData " should "return the external data for the identifier " in {
    ...
    val res: Option[ObjectExtData] = new TestQuestionPublisher().getExternalData(identifier, 0.0, "", readerConfig)
